...
TEST(OrderBook, CancelUnknownIdIsNoOp) {
	order_book ob;
	(void)ob.place_order({.id = 1, .side = side::bid, .price = 100, .qty = 10});
...
TEST(OrderBook, CancelOneOfTwoAtSameLevelKeepsTheOther) {
	order_book ob;
	(void)ob.place_order({.id = 1, .side = side::bid, .price = 100, .qty = 10});
	(void)ob.place_order({.id = 2, .side = side::bid, .price = 100, .qty = 7});
...
TEST(OrderBook, CrossingOrderFullyFillsAndEmptiesBook) {
	order_book ob;
	(void)ob.place_order(
		{.id = 1, .side = side::ask, .price = 100, .qty = 10}); // rests
	const auto trades = ob.place_order(
		{.id = 2, .side = side::bid, .price = 100, .qty = 10});
...
TEST(OrderBook, PartialCrossRestsRemainderOnAggressorSide) {
	order_book ob;
	(void)ob.place_order({.id = 1, .side = side::ask, .price = 100, .qty = 5});
	const auto trades =
		ob.place_order({.id = 2, .side = side::bid, .price = 100, .qty = 8});
...
TEST(OrderBook, MatchingHonoursTimePriority) {
	...
	(void)ob.place_order(
		{.id = 2, .side = side::ask, .price = 100, .qty = 5}); // second

	const auto trades =
		ob.place_order({.id = 3, .side = side::bid, .price = 100, .qty = 5});
...
TEST(OrderBook, CrossingSweepsMultipleLevelsUpToLimit) {
	order_book ob;
	(void)ob.place_order({.id = 1, .side = side::ask, .price = 100, .qty = 5});
	(void)ob.place_order({.id = 2, .side = side::ask, .price = 101, .qty = 5});
	(void)ob.place_order({.id = 3, .side = side::ask, .price = 102, .qty = 5});

	const auto trades =
		ob.place_order({.id = 4, .side = side::bid, .price = 101, .qty = 8});
...
TEST(OrderBook, FillOrKillExecutesWhenLiquiditySufficient) {
	order_book ob;
	(void)ob.place_order({.id = 1, .side = side::ask, .price = 100, .qty = 10});
	const auto trades = ob.place_order({.id     = 2,
										.side   = side::bid,
										.price  = 100,
										.qty = 8,
