...
// reordered - and both are only interesting when the ring fills, so most of
// this file works with a ring of four.
...
// A listing's trades precede its outcomes, so a reader can apply the order
// state to the execution that caused it rather than the other way round. This
// is the property two separate rings could not offer, and the reason there is
// one.
...
// first four go, the remainder is held, and retry finishes the job once the
// host has made room. Nothing is dropped and nothing is reordered - a full ring
// is back-pressure, never eviction, because a lost channel_print would make the
// tape a function of scheduling.
...
// retry() with no backlog is vacuously done, so a caller can put it at the top
// of its loop without asking whether it is needed.
