...
// --------------------------------------------------------------------------
// The two operations that turn a stamp into the scalars a field wants
// --------------------------------------------------------------------------

TEST(EventLifecycle, EpochNanosIsTheStampsOwnCount) {
	EXPECT_EQ(epoch_nanos(WHEN), 1'700'000'000'000'000'000LL);
}

TEST(EventLifecycle, EpochNanosStaysSignedBeforeTheEpoch) {
	// The documented reason the return type is not widened to session_id_t's
	// unsigned: an instant before 1970 is representable, and should read as
	// negative rather than as an enormous positive.
	constexpr wall_time BEFORE{std::chrono::nanoseconds{-1}};
	EXPECT_EQ(epoch_nanos(BEFORE), -1);
	EXPECT_LT(epoch_nanos(BEFORE), 0);
}

TEST(EventLifecycle, EpochNanosIsUsableAtCompileTime) {
	// Not decoration: a formatter calls it on a constant expression and a
	// constexpr guarantee is what keeps that free.
	static_assert(epoch_nanos(wall_time{std::chrono::nanoseconds{42}}) == 42);
	SUCCEED();
}

TEST(EventLifecycle, SessionOfDistinguishesTwoOpenings) {
	// The first half of session_id_t's contract - consecutive ids differ -
	// which is what makes a client order id unique within a session.
	constexpr wall_time FIRST{std::chrono::nanoseconds{1000}};
	constexpr wall_time SECOND{std::chrono::nanoseconds{1001}};
	EXPECT_NE(session_of(FIRST), session_of(SECOND));
}

TEST(EventLifecycle, SessionOfIsMonotonicWithTheClock) {
	// The second half, and the one a reader leans on: a replay walking a log
	// backwards can stop at the first session older than the one it wants
	// without decoding what follows.
	constexpr wall_time EARLIER{std::chrono::nanoseconds{1000}};
	constexpr wall_time LATER{std::chrono::nanoseconds{2000}};
	EXPECT_LT(session_of(EARLIER), session_of(LATER));
}

TEST(EventLifecycle, SessionOfAgreesWithTheStampItCameFrom) {
	// Derived, not invented: the id is the reading, so a record's own timestamp
...
// else" are different questions and each has its own spelling. This is the
// whole reason the combination is not a third enumerator.
...
// A replay from an empty book is the case worth alerting on, and it is the one
// a bare JOURNAL bit names.
...
// "rebuilt out of nothing", which is a cold start, which emits no recovery
// record at all. is_well_formed is what says so.
