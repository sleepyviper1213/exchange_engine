...
TEST(ObjectPool, ReportsConstructedCapacity) {
	object_pool<object_payload> pool(64);
	EXPECT_EQ(pool.size(), 64u);
}

TEST(ObjectPool, AllocateReturnsUsableStorage) {
	object_pool<object_payload> pool(4);
	object_payload *p = pool.allocate();
	ASSERT_NE(p, nullptr);
...
TEST(ObjectPool, DrainsExactlyCapacityDistinctObjects) {
	...
	object_pool<object_payload> pool(cap);

	std::unordered_set<object_payload *> seen;
	seen.reserve(cap);
	for (std::uint32_t i = 0; i < cap; ++i) {
		object_payload *p = pool.allocate();
		ASSERT_NE(p, nullptr);
		EXPECT_TRUE(seen.insert(p).second)
			<< "slot handed out twice at i=" << i;
	}
	for (object_payload *p : seen) pool.free(p);
}
...
TEST(ObjectPool, FreedSlotIsReused) {
	...
	std::vector<object_payload *> all;
	std::unordered_set<object_payload *> pooled;
	for (std::uint32_t i = 0; i < cap; ++i) {
		object_payload *p = pool.allocate();
		all.push_back(p);
		pooled.insert(p);
	}
	for (object_payload *p : all) pool.free(p);
	...
	for (std::uint32_t i = 0; i < cap; ++i) {
		...
		EXPECT_TRUE(pooled.count(p) == 1)
			<< "allocation " << i
			<< " left the pool's cells while slots were free";
	}
...
TEST(ObjectPool, ExhaustionReturnsNullRatherThanAllocating) {
	...
	std::vector<object_payload *> held;
	for (std::uint32_t i = 0; i < cap; ++i) {
		object_payload *p = pool.allocate();
	...
	object_payload *reused = pool.allocate();
	ASSERT_NE(reused, nullptr);
	EXPECT_EQ(pool.allocate(), nullptr);

	held.push_back(reused);
	for (object_payload *p : held) pool.free(p);
...
TEST(ObjectPool, FreeingNullIsANoOp) {
	...
	object_pool<object_payload> pool(2);
	pool.free(nullptr);
...
TEST(ObjectPool, AvailableTracksOutstanding) {
	object_pool<object_payload> pool(8);
	EXPECT_EQ(pool.available(), 8u);
	object_payload *a = pool.allocate();
	object_payload *b = pool.allocate();
...
// Note: object_pool is single-threaded by contract (one pool per owning
// thread), so there is no concurrency test - sharing a pool across threads is a
// usage error, not a case to verify.
