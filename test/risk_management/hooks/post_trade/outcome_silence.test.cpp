...
TEST(PostTradeOutcomeSilence, TripsOnSilenceWithExposure) {
	...
	EXPECT_FALSE(rule.poll(POST_TRADE_TIMEOUT_NS, 1))
		<< "exactly the timeout is still admissible";
	EXPECT_TRUE(rule.poll(POST_TRADE_TIMEOUT_NS + 1, 1));
...
TEST(PostTradeOutcomeSilence, IdleAndSilentIsNotAFault) {
	...
	EXPECT_FALSE(rule.poll(POST_TRADE_TIMEOUT_NS * 1000, 0));
	EXPECT_FALSE(rule.is_silent(POST_TRADE_TIMEOUT_NS * 1000, 0));
...
TEST(PostTradeOutcomeSilence, AnOutcomeIsEvidenceThePathIsAlive) {
	...
	EXPECT_FALSE(rule.poll(nearly, 1));

	rule.beat(nearly);
	EXPECT_EQ(rule.last_outcome_ns(), nearly);
	EXPECT_EQ(rule.silence_ns(nearly), 0U);
	EXPECT_EQ(rule.outcomes(), 1U);

	EXPECT_FALSE(rule.poll(nearly + POST_TRADE_TIMEOUT_NS, 1))
		<< "the clock restarted from the beat, not from construction";
	EXPECT_TRUE(rule.poll(nearly + POST_TRADE_TIMEOUT_NS + 1, 1));
}
...
TEST(PostTradeOutcomeSilence, ADisabledTimeoutNeverTrips) {
	...
	EXPECT_FALSE(rule.poll(1'000'000'000'000, 1000));
	EXPECT_FALSE(rule.is_silent(1'000'000'000'000, 1000));
...
TEST(PostTradeOutcomeSilence, AReadingBehindTheLastOutcomeIsNoSilenceAtAll) {
	...
	EXPECT_EQ(rule.silence_ns(1000), 0U);
	EXPECT_FALSE(rule.poll(1000, 10));
	EXPECT_EQ(breaker.state(), trading_state::NORMAL);
}
...
TEST(PostTradeOutcomeSilence, OneOutageTripsOnce) {
	...
	EXPECT_TRUE(rule.poll(past, 1));
	for (int i = 0; i < 10; ++i) EXPECT_FALSE(rule.poll(past, 1));
	EXPECT_EQ(rule.trips(), 1U);
}
...
TEST(PostTradeOutcomeSilence, AMonitorNobodyPollsNeverFires) {
	...
	EXPECT_TRUE(rule.is_silent(POST_TRADE_TIMEOUT_NS * 2, 1));
	EXPECT_EQ(breaker.state(), trading_state::NORMAL);
