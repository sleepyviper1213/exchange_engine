...
TEST(RiskRateLimiter, ChargingSpendsHeadroomWithinTheWindow) {
	...
	EXPECT_EQ(limiter.headroom(at_ns(WINDOW_NS - 1)), 2U);
	EXPECT_FALSE(limiter.admits(at_ns(WINDOW_NS - 1), 3));
}
...
TEST(RiskRateLimiter, CrossingAWindowBoundaryRestoresTheAllowance) {
	rate_limiter limiter{2, SMALL_WINDOW_LOG2};
	limiter.charge(at_ns(0), 2);
	ASSERT_EQ(limiter.headroom(at_ns(WINDOW_NS - 1)), 0U);
	EXPECT_EQ(limiter.headroom(at_ns(WINDOW_NS)), 2U);
	EXPECT_EQ(limiter.used(at_ns(WINDOW_NS)), 0U);
}
...
TEST(RiskRateLimiter, ALimiterLeftAloneForManyWindowsReportsUnused) {
	...
	EXPECT_EQ(limiter.used(at_ns(WINDOW_NS * 1000)), 0U);
	EXPECT_EQ(limiter.headroom(at_ns(WINDOW_NS * 1000)), 4U);
}
...
TEST(RiskRateLimiter, AFixedWindowAdmitsTwiceTheLimitAcrossOneBoundary) {
	...
	limiter.charge(at_ns(WINDOW_NS - 1), 3);
	EXPECT_EQ(limiter.headroom(at_ns(WINDOW_NS - 1)), 0U);
	EXPECT_EQ(limiter.headroom(at_ns(WINDOW_NS)), 3U);
	limiter.charge(at_ns(WINDOW_NS), 3);
	// Six messages inside two nanoseconds, against a limit of three per window.
	EXPECT_EQ(limiter.used(at_ns(WINDOW_NS)), 3U);
}
...
TEST(RiskRateLimiter, ResetForgetsTheCurrentWindow) {
	rate_limiter limiter{4, SMALL_WINDOW_LOG2};
	limiter.charge(at_ns(WINDOW_NS * 7), 4);
	ASSERT_EQ(limiter.headroom(at_ns(WINDOW_NS * 7)), 0U);
	limiter.reset();
	EXPECT_EQ(limiter.headroom(at_ns(WINDOW_NS * 7)), 4U);
}
