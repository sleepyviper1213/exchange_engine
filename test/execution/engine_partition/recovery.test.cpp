...
// Recovery, end to end, against the real thing: a store, the replay driver, and
// a partition whose command queue is deliberately far too small.
//
// The small queue is the point. `submit` refuses when its ring is full, so a
// recovery that replays a journal into a live partition *will* be refused part
// way through - routinely, not exceptionally, because the disk is faster than
// the consumer. Every other test of replay uses an applier that always accepts,
// which is the easy half. This one uses the applier recovery actually has.
...
std::vector<command> recovery_mixed_flow() {
...
/// @brief Drain until nothing is left, so the queue is empty and has room
/// again.
...
// The original run and a recovery from its journal must agree on every trade
// and every outcome. Not on counts - on the records, in order.
TEST(EnginePartitionRecovery, ReplayingAStoresJournalReproducesTheRun) {
	...
	const std::vector<command> flow = recovery_mixed_flow();
	...
	for (;;) {
		const auto step =
			replay(store->journal(), at, [&](const journal_record &record) {
	...
	EXPECT_GT(refusals, 0U)
		<< "a 4-slot queue should have refused a 15-command "
...
// so a driver that got this wrong would be loudly wrong, which is what this
// pins.
TEST(EnginePartitionRecovery, ACheckpointsSequenceIsNotReplayed) {
...

	{
		...
		// A checkpoint covering the first four records and no more. The
		// snapshot file stands in for the book state as of that record, which
		// persistence cannot write itself - and the sequence is the caller's to
		// state, which is what lets this test say "four" while the journal
		// holds fifteen.
	...
	// The record right after the checkpoint, not the one before it: an
	// off-by-one here is a duplicate PLACE on the way back up.
...
TEST(EnginePartitionRecovery, ACheckpointAtTheTailLeavesNothingToReplay) {
...

	{
		...
		out << "state";
		out.close();
		ASSERT_TRUE(store->commit(1, store->journal().count(), /*session=*/3)
						.has_value());
	}
...
// A checkpoint claiming records the journal does not hold is refused where it
// is still preventable. Accepted, it would send recovery past the end of the
// log and report a clean start having replayed nothing - the books quietly
// missing every command the snapshot did not contain.
