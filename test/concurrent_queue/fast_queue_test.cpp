...
/// @brief Pop one message into a string, or std::nullopt if the queue is empty.
...
TEST(FastQueue, SpscThreadedRoundTripDeliversEveryMessageInOrder) {
    ...
    std::thread producer([&] {
        for (int i = 0; i < kCount; ++i) {
            ...
            while (!q.try_push(payload)) {
                std::this_thread::yield(); // queue full — let the consumer catch up
            }
        }
    });
    ...
    while (received < kCount) {
        ...
        const auto n = q.try_pop(buf);
        if (!n) {
            std::this_thread::yield(); // queue empty — wait for the producer
            continue;
        }
