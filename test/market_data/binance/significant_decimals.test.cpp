...
TEST(BinanceSignificantDecimals, TheTrimmedFormGivesTheSameAnswer) {
	...
	EXPECT_EQ(significant_decimals("0.001"), significant_decimals("0.00100000"));
...
TEST(BinanceSignificantDecimals, NonsenseIsZeroRatherThanUndefined) {
	// Never throws and never reads past the end - it is noexcept, and it runs on
	// whatever a venue sent. Nothing here is a *valid* increment; the contract
	// is only that each has an answer.
