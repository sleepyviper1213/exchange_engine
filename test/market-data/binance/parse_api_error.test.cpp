#include "market-data/binance/api_error.hpp"
...
// whatever the HTTP status. The status says which class of thing went wrong; only
// the body says what, so reporting the status alone leaves an operator holding
// "HTTP 400" and a JSON blob.
...
TEST(BinanceParseApiError, TheEnvelopeTheVenueActuallySends) {
	...
	const auto parsed = parse_api_error(R"({"code":-1121,"msg":"Invalid symbol."})");
...
TEST(BinanceParseApiError, WhatIsNotJsonIsNotAnEnvelope) {
	...
	EXPECT_FALSE(parse_api_error("<html><body>403 Forbidden</body></html>")
					 .has_value());
...
TEST(BinanceDescribeApiError, TheFallbackIsUsedWhenThereIsNoEnvelope) {
	EXPECT_EQ(describe_api_error("<html>502</html>", "HTTP 502: <html>502</html>"),
