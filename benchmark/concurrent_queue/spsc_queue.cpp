...
BENCHMARK(BM_SPSC_Queue_MultiThreaded_Throughput<int>)
->Unit(benchmark::kNanosecond)->Repetitions(10)->DisplayAggregatesOnly(true);
...
BENCHMARK(BM_Fifo5b_MultiThreaded_Throughput<int>)
->Unit(benchmark::kNanosecond)->Repetitions(10)->DisplayAggregatesOnly(true);
...
BENCHMARK(BM_SPSC_Queue_OutParam_SingleThread_RoundTrip<int>)
->Unit(benchmark::kNanosecond)->Repetitions(10)->DisplayAggregatesOnly(true);
...
BENCHMARK(BM_Fifo5b_SingleThread_RoundTrip<int>)
->Unit(benchmark::kNanosecond)->Repetitions(10)->DisplayAggregatesOnly(true);
...
void BM_SPSC_Queue_BatchEmplaceRange_Throughput(benchmark::State &state) {
	...
	for (size_t i = 0; i < batch; ++i) payload[i] = static_cast<Q>(i);
