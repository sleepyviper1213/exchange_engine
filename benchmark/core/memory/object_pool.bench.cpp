...
// steady-state per-op cost on a single core and (2) whether that cost holds
// when every core drives its own pool. new/delete is benchmarked alongside as
// the reference the pool has to beat to justify existing.
namespace {

using exchange::core::memory::object_pool;

// A representative resting-order node: a handful of 8-byte fields, ~40 bytes,
// so the measurement reflects moving a real node-sized object rather than an
// int.
struct pooled_order {
	std::uint64_t id;
	std::uint64_t price;
...
// Sized well above any working set these cases touch, so allocate() always has
// a free cell and never returns null (which would measure a branch instead of
// the pool). The pool imposes no shape on its capacity; a power of two is just
// a round number here.
...
void BM_ObjectPool_ST_AllocFree(benchmark::State &state) {
	...
	for (auto _ : state) {
		...
		obj->id =
			1; // touch the node so the round-trip cannot be optimized away
...
// captures the cost once the working set spills L1/L2 and the link chasing
// stops hitting cache. N is capped at capacity, so every allocation still finds
// a free cell. The pointer vector is hoisted out of the timed loop so only
...
// not the pool. UseRealTime because wall-clock is the throughput signal across
// threads.
void BM_ObjectPool_PerThreadPool_AllocFree(benchmark::State &state) {
	object_pool<pooled_order> pool(
		kPoolCapacity); // one pool per benchmark thread
...
BENCHMARK(BM_ObjectPool_PerThreadPool_AllocFree)
