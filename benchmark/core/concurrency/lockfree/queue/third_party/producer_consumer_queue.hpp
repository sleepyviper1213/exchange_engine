...
// Vendored from folly/ProducerConsumerQueue.h. The only local change is
// dropping the #include <folly/concurrency/CacheLocality.h> dependency and
// sourcing hardware_destructive_interference_size from <new> instead, so the
// header is fully self-contained and needs none of the rest of folly (whose
// vcpkg port does not build cleanly under the MinGW triplet this project uses).
// Kept close to verbatim (like the moodycamel headers) so it remains a faithful
// reference point in the SPSC benchmark shoot-out.
