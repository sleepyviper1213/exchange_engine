...
#pragma warning(                                                               \
	disable                                                                    \
	: 4100) // Get rid of (erroneous) 'unreferenced formal parameter' warning
#endif
	template <typename U>
...
#pragma warning(pop)
	...
	AE_FORCEINLINE const weak_atomic &
	operator=(const weak_atomic &other) AE_NO_TSAN {
		value = other.value;
		return *this;
	}
...
#if defined(AE_ARCH_X64) || defined(AE_ARCH_X86)
		if (sizeof(T) == 4)
			return _InterlockedExchangeAdd((volatile long *)&value,
										   (long)increment);
#if defined(_M_AMD64)
		else if (sizeof(T) == 8)
			return _InterlockedExchangeAdd64((volatile long long *)&value,
											 (long long)increment);
#endif
#else
...
extern "C" {
	...
	CreateSemaphoreW(_SECURITY_ATTRIBUTES *lpSemaphoreAttributes,
...
public:
	AE_NO_TSAN Semaphore(int initialCount = 0) : m_hSema() {
		...
		const long maxLong = 0x7fff'ffff;
	...
	bool wait() AE_NO_TSAN {
		const unsigned long infinite = 0xffff'ffff;
...
public:
	...
	bool timed_wait(std::uint64_t timeout_usecs) AE_NO_TSAN {
		mach_timespec_t ts;
		ts.tv_sec  = static_cast<unsigned int>(timeout_usecs / 1'000'000);
		ts.tv_nsec = static_cast<int>((timeout_usecs % 1'000'000) * 1000);
...
public:
	...
	bool timed_wait(std::uint64_t usecs) AE_NO_TSAN {
		struct timespec ts;
		const int usecs_in_1_sec = 1'000'000;
		const int nsecs_in_1_sec = 1'000'000'000;
