...
public:
	...
	explicit Fifo5(size_type capacity, const Alloc &alloc = Alloc{})
		: Alloc{alloc},
		  capacity_{capacity},
		  ring_{allocator_traits::allocate(*this, capacity)} {}

	~Fifo5() { allocator_traits::deallocate(*this, ring_, capacity()); }
	...
	public:
		...
		explicit pusher_t(Fifo5 *fifo, size_type cursor) noexcept
		...
		pusher_t(pusher_t &&other) noexcept
			: fifo_{std::move(other.fifo_)}, cursor_{std::move(other.cursor_)} {
			other.release();
		}
		...
		~pusher_t() {
			if (fifo_) {
				fifo_->pushCursor_.store(cursor_ + 1,
		...
		pusher_t &operator=(const value_type &value) noexcept {
			std::memcpy(get(),
						std::addressof(value),
						ValueSizeTraits<value_type>::size(value));
			return *this;
		}
	...
	pusher_t push() noexcept {
		...
		if (full(pushCursor, popCursorCached_)) {
			...
			if (full(pushCursor, popCursorCached_)) return pusher_t{};
		}
	...
	public:
		...
		explicit popper_t(Fifo5 *fifo, size_type cursor) noexcept
			: fifo_{fifo}, cursor_{cursor} {}
	...
	auto pop() noexcept {
		...
		if (empty(pushCursorCached_, popCursor)) {
			...
			if (empty(pushCursorCached_, popCursor)) return popper_t{};
		}
...
private:
	...
	const auto *element(size_type cursor) const noexcept {
		return &ring_[cursor % capacity_];
	}
...
private:
	...
	// See Fifo3.hpp for reason why std::hardware_destructive_interference_size
...
public:
	...
	explicit Fifo5b(size_type capacity, const Alloc &alloc = Alloc{})
		: Alloc{alloc},
		  mask_{capacity - 1},
		  ring_{allocator_traits::allocate(*this, capacity)} {
	...
	Fifo5b(const Fifo5b &) = delete;

	Fifo5b(Fifo5b &&) = delete;

	~Fifo5b() { allocator_traits::deallocate(*this, ring_, capacity()); }
	...
	public:
		...
		explicit pusher_t(Fifo5b *fifo, size_type cursor) noexcept
			: fifo_{fifo}, cursor_{cursor} {}
		...
		~pusher_t() {
			if (fifo_) {
				fifo_->pushCursorRef_.store(cursor_ + 1,
											std::memory_order_release);
			}
		}
	...
	public:
		...
		explicit popper_t(Fifo5b *fifo, size_type cursor) noexcept
			: fifo_{fifo}, cursor_{cursor} {}
		...
		~popper_t() {
			if (fifo_) {
				fifo_->popCursorRef_.store(cursor_ + 1,
										   std::memory_order_release);
			}
		}
