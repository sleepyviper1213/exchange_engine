...
// local-order-book procedure Binance documents for the `<symbol>@depth` feed.
// Each diff level is an *absolute* aggregated size (0 = remove), applied via
// binance::apply_depth_update. The feed is offline and deterministic so the
// timed region has no network or JSON cost (see replay_data.hpp for the input).
namespace {
    /**
