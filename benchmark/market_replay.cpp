...
// `<symbol>@depth` WebSocket feed. Each diff level is an *absolute* aggregated
// size (0 = remove), applied via OrderBook::set_level.
//
// The feed is offline and deterministic so the timed region has no network or
// JSON cost. Point OB_REPLAY at a JSONL capture of depthUpdate frames (one per
// line, e.g. from `websocat`); otherwise a representative stream is
// synthesized. Point OB_SNAPSHOT at a saved REST depth JSON for the seed book;
// otherwise a SOLUSDT- shaped book is synthesized. OB_PRICE_DECIMALS /
// OB_QTY_DECIMALS override the tick/step precision used to parse both files
// (default 2, matching SOLUSDT).
namespace {
...
synth_updates(const binance::DepthSnapshot& seed) {
    ...
    for (std::size_t e = 0; e < kSynthEvents; ++e) {
        ...
        for (std::size_t k = 0; k < kSynthTouchPerSide; ++k) {
            u.bids.emplace_back(bid_ref - off(rng), qty(rng));
            u.asks.emplace_back(ask_ref + off(rng), qty(rng));
        }
 ...
 */
std::vector<binance::DepthUpdate> updates(const binance::DepthSnapshot& seed,
                                          int price_decimals,
                                          int qty_decimals) {
    if (const char* path = std::getenv("OB_REPLAY")) {
        auto parsed = binance::parse_binance_depth_updates(
            slurp(path), price_decimals, qty_decimals);
        if (!parsed) {
            std::abort();
        }
        return *parsed;
...
ReplayData load() {
    ...
    ReplayData data{.snap = snapshot(pd, qd),
                    .feed = updates(data.snap, pd, qd)};
