...
[[nodiscard]] bool hit(const price_level *data, std::size_t size,
					   std::size_t at, scaled_price_t price) noexcept {
...
void l2_book::load(side_t side, std::span<const price_level> levels) {
	...
	const auto copied =
		is_bid ? std::ranges::partial_sort_copy(positive,
												window,
												std::greater<>{},
												&price_level::price,
												&price_level::price)
...
void l2_book::load(side_t side, std::initializer_list<price_level> levels) {
...
l2_book::bid_levels() const noexcept {
	return {bids(), bid_size_};
}

[[nodiscard]] std::span<const price_level>
l2_book::ask_levels() const noexcept {
	return {asks(), ask_size_};
}

depth_sweep l2_book::sweep(const price_level *levels, std::size_t count,
						   side_t side, scaled_qty_t size) noexcept {
	...
	result.touch = levels[0].price;
	result.last  = levels[0].price;

	scaled_qty_t left = size;
	for (std::size_t at = 0; at < count && left > 0; ++at) {
		...
		assert(levels[at].qty > 0 && "a non-positive size is not a level");

		const scaled_qty_t taken = std::min(left, levels[at].qty);
		left -= taken;
		result.filled += taken;
		result.last = levels[at].price;
		++result.levels;
	}
	return result;
}
...
depth_sweep l2_book::sweep_asks(scaled_qty_t size) const noexcept {
	return sweep(asks(), ask_size_, side_t::ask, size);
}

depth_sweep l2_book::sweep_bids(scaled_qty_t size) const noexcept {
	return sweep(bids(), bid_size_, side_t::bid, size);
}
...
[[nodiscard]] l2_book::price_level *l2_book::bids() noexcept {
	return cells_.get();
}
