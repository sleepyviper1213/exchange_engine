...
public:
	...
	[[nodiscard]] bool is_alive() const noexcept {
		return sequencer_.is_streaming();
	}
	...
	/**
	 * @brief Ingress stamp of the oldest event the last @c on_snapshot
	 * replayed, or a default stamp if it replayed none.
	 *
	 * The one number a resync's cost is not otherwise visible in. A snapshot
	 * brings the replica back live and the caller reacts to that, but what it
	 * is reacting to includes every buffered event replayed underneath it - and
	 * the oldest of those arrived one whole fetch round trip ago. Measuring the
	 * reaction from the snapshot's own arrival would therefore report the fast
	 * half and hide the slow one.
	 *
	 * Reset at the start of every @c on_snapshot, so it describes that call and
	 * not an earlier one. @see core::chrono::has_ingress
	 */
	[[nodiscard]] core::chrono::ingress_time
	last_replay_ingress() const noexcept {
		return replay_ingress_;
	}
...
private:
	...
	/// Oldest ingress replayed by the last on_snapshot. @see
	/// last_replay_ingress
	/// @c {} to match the fields it is compared against; see the note on
	/// @c depth_event::ingress for why it is kept despite being redundant.
	/// NOLINTNEXTLINE(readability-redundant-member-init)
	core::chrono::ingress_time replay_ingress_{};
