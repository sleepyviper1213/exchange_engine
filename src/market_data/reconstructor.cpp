...
bool depth_reconstructor::on_snapshot(const book_snapshot &snapshot) {
	...
	// fetches outstanding with the older one landing second is an ordinary way
	// to get here, not a pathological one, so it is refused rather than left
	// for the gap that some later event would eventually trip.
	...
	snapshot_pending_ = false;
	// Describes this call only - a caller reading it after a snapshot that
	// bridged nothing must see "nothing was replayed", not the last one that
	// did. @see last_replay_ingress
	replay_ingress_           = core::chrono::ingress_time{};
	...
	while (!pending_.empty()) {
		...
		if (action == sequence_action::apply) {
			// First applied is the oldest: the deque is in arrival order and
			// everything ahead of it dropped out as already covered.
			if (!core::chrono::has_ingress(replay_ingress_))
				replay_ingress_ = event.ingress;
			apply(book_, event);
		}
		pending_.pop_front();
	}
