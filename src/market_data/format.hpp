...
inline std::string level_text(const l2_book::price_level &level,
...
// l2_book::price_level, so the specialisation above already covers it.
// Declaring both would be a redefinition of the same specialisation, which is
// how the alias makes "a diff level drops straight into the book" true of the
// printing too.
...
/**
 * @brief A depth sweep as
 *        @c "sweep[ask 500/500 lots, 3 levels, @15000 -> @15004, impact 4]",
 *        or @c "sweep[ask 320/500 lots INCOMPLETE, ...]" when the published
 *        depth could not cover the size.
 *
 * The requested size prints beside the filled one even when they agree, because
 * the interesting reading of this record is usually the ratio and a line that
 * shows only what filled cannot be compared against the line before it.
 * @see depth_sweep on why there is no average price here to print.
 */
template <>
struct fmt::formatter<exchange::market_data::depth_sweep>
	: fmt::nested_formatter<std::string_view> {
	auto format(const exchange::market_data::depth_sweep &sweep,
				format_context &ctx) const -> format_context::iterator {
		return write_padded(ctx, [&](auto out) {
			out = fmt::format_to(out,
								 "sweep[{} {}/{}",
