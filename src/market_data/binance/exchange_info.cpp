 ...
 * @c "0.01000000" where the increment is one hundredth. That form cannot be used
 * as-is: @c parse_exact_decimal requires a decimal to be *exactly* representable
 * at the scale it is given, so eight decimals against a scale of two is refused
 * as malformed - correctly, since silently dropping digits is how a grid ends up
 * wrong. Trimming here is not rounding: every digit removed is a zero.
 *
...
parse_exchange_info(std::string_view json, std::string_view symbol) {
	...
	if (const auto err = parser.parse(simdjson::padded_string(json)).get(doc))
		return std::unexpected(
			fmt::format("exchangeInfo is not JSON: {}", simdjson::error_message(err)));
	...
	// which for a request scoped to one symbol means the venue does not have it.
	return std::unexpected(
