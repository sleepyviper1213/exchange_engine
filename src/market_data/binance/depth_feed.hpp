...
//
// This is the whole of what market_data/feed.hpp asked for, filled in for one
...
public:
	 ...
	 *        the returned event. Default-constructed - the "nobody stamped
	 ...
	 * Because this is not where the bytes arrived. A frame reaches this call
	 * after a socket read completed, a coroutine resumed and a buffer was
	 * handed over; reading the clock here would fold all of that into the
	 * decode cost and leave whoever owns the socket unable to say otherwise.
	 * Whoever obtains the bytes owns their arrival time, the same division the
	 * class comment already draws between the route and the meaning.
	 */
...
public:
	 ...
	 * @note Events come back with no ingress stamp, deliberately. A capture's
	 *       frames arrived when they were recorded, not when they are read
	 * back, so stamping them here would report the replay loop's own speed as a
	 *       reaction time - a number that gets *better* the further the harness
	 *       drifts from the live path it stands in for. A replay cannot measure
	 *       a reaction it did not have. @see core::chrono::ingress_clock
	 */
