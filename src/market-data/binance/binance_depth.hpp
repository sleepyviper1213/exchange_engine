 ...
 *
 * Each level in @p update is an absolute aggregated size, so it maps directly to
 * @c OrderBook::set_level; a level whose volume is 0 removes that price. This is
 * the per-event step of the managed-local-order-book replay (seed from a REST
 * snapshot, then stream diffs through this).
 ...
 *
 * The free @c parse_binance_depth* functions construct a fresh simdjson parser and
 * a fresh padded input buffer on every call — fine for one-shot use, wasteful when
 * decoding a @c \@depth WebSocket stream frame after frame. This owns both across
 * calls: simdjson amortizes its internal structural-index/tape buffers, and the
 * input buffer's allocation is reused (only regrown when a frame is larger than any
 * seen so far). On a steady feed this removes the per-frame allocations that
 * dominate tick-to-book latency.
 *
 * @note Stateful and @b not thread-safe — use one instance per consuming thread.
 *       Each returned view/snapshot is independent of the parser's buffers (levels
