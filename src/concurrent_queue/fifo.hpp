...
struct ValueSizeTraits {
    ...
    static std::size_t size(value_type const &value) {
...
public:
    ...
    public:
        ...
        explicit pusher_t(Fifo5 *fifo, size_type cursor) noexcept : fifo_{fifo},
            cursor_{cursor} {
        }
        ...
        ~pusher_t() {
            if (fifo_) {
                fifo_->pushCursor_.
                        store(cursor_ + 1, std::memory_order_release);
            }
        }
        ...
        pusher_t &operator=(value_type const &value) noexcept {
            std::memcpy(get(), std::addressof(value),
    ...
    public:
        ...
        explicit popper_t(Fifo5 *fifo, size_type cursor) noexcept : fifo_{fifo},
            cursor_{cursor} {
        }
...
private:
    ...
    auto const *element(size_type cursor) const noexcept {
        return &ring_[cursor % capacity_];
    }
...
private:
    ...
    static constexpr auto hardware_destructive_interference_size = size_type
            {64};
...
public:
    ...
    public:
        ...
        explicit
        pusher_t(Fifo5b *fifo, size_type cursor) noexcept : fifo_{fifo},
            cursor_{cursor} {
        }
    ...
    public:
        ...
        explicit
        popper_t(Fifo5b *fifo, size_type cursor) noexcept : fifo_{fifo},
            cursor_{cursor} {
        }
