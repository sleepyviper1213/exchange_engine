...
[[nodiscard]] bool is_known_tag(std::uint8_t tag) noexcept {
	switch (static_cast<command::Type>(tag)) {
	case command::Type::PLACE:
	case command::Type::CANCEL:
	case command::Type::ADD:
	case command::Type::REDUCE: return true;
	}
	return false;
}
...
/// @brief Decode an ADD or REDUCE, whose payload is a side, a price and a size.
[[nodiscard]] std::expected<command, std::string>
decode_level(const journal_record &record, command::Type tag) {
	const std::optional<side_t> side = to_side(byte_at(record, AT_SIDE));
	if (!side)
		return std::unexpected(
			fmt::format("side byte {} names neither side of the book",
...
decode(const journal_record &record) noexcept try {
	...
	switch (static_cast<command::Type>(tag)) {
	case command::Type::PLACE: return decode_place(record);
	case command::Type::CANCEL:
	...
	case command::Type::ADD:
	case command::Type::REDUCE:
		return decode_level(record, static_cast<command::Type>(tag));
	}
