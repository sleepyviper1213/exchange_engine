 ...
 * forever after: @c acquire() constructs a @c T in a free node and hands it out,
 ...
 * @c head==A and @c A->next, is stalled, and by the time it CASes, @c A has been
 * popped and pushed again with a different @c next — the CAS wrongly succeeds. A
 * hazard pointer published on @c A does @b not by itself close this window: a
 * hazard pointer guards *reclamation* (it stops the node's memory being freed),
 * not the CAS *identity*. In a naive freelist that pushes a released node
 * straight back, nothing is ever freed, so the guard is vacuous and ABA remains.
 *
 ...
 * hazard pointer protecting the node, so a node that any @c acquire() is mid-pop
 * on cannot re-enter the free stack until that acquirer clears its hazard
 * pointer. The head therefore cannot transition A → … → A within a single
 * acquire's critical section, and ABA is impossible. This is the whole point of
 * the design: the hazard pointer on the popped head, combined with
 ...
 * checked out (a live @c T constructed in it), or retired-pending (released, its
 ...
 * sweep returns it. @c acquire() can thus report exhaustion (returns @c nullptr)
 * while released nodes are still draining back. Call @c quiesce() in a quiescent
 * phase to force them home.
 */
template <class T>
class freelist {
...
public:
	...
	explicit freelist(std::size_t capacity)
		...
		for (std::size_t i = 0; i < capacity_; ++i) push(free_head_, &nodes_[i]);
	 ...
	 * @pre Quiescent: no thread is acquiring, releasing, or holding a checked-out
	 * element. Any @c T still checked out is the caller's to release first.
	 * @details Member destruction runs in reverse declaration order, so @c domain_
	 * is torn down first: it reclaims every retired-pending node, whose recycler
	 * harmlessly pushes it onto @c free_head_ (still alive). @c nodes_ is destroyed
	 * last, freeing the storage exactly once — the recycler never frees, so there
	 * is no double free.
	 */
	~freelist() = default;
	...
	[[nodiscard]] T *acquire(Args &&...args) {
		...
		if (n == nullptr) [[unlikely]] {
			// The free stack looks empty, but released nodes may still be in the
			// retired-pending state: reclamation is batched, and the domain's
			// threshold can exceed a small pool's capacity, so a sweep might not
			// have fired yet. Force one and retry once before reporting
			// exhaustion. This still recycles only through the hazard-pointer
			// gate, so the ABA guarantee is unaffected.
			domain_.cleanup();
			n = pop();
	 ...
	 * recycling; it becomes acquirable again once no hazard pointer protects it.
	 ...
	 * internally), so it may be called under concurrent acquire/release. It only
	 * recovers nodes no hazard pointer currently protects; a node another thread
	 * is mid-acquire on stays pending until that thread clears its protection.
	 * @post Every released node not currently protected is back on the free
	 * stack. With no concurrent activity, the pool's full capacity is acquirable.
	 */
