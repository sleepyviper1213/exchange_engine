...
// @see fwd.hpp for how this relates to wall and ingress time.
 ...
 * Every time-dependent decision in the engine - a gate's rate window, a
 ...
 *
 * Driving the same clock from a depth event's venue timestamp fixes both halves
 * at once. Intervals come out in market time, so the limits fire where they
 * would have fired live; and the run becomes a pure function of the capture, so
 * two runs of the same file on two machines produce identical reports. That
 * determinism is the property a backtest is *for* - without it a result cannot
 * be attributed to a change in the strategy.
 *
 * @c nanosecond_clock exists precisely so this can be substituted; its own
 * documentation names "a replay driving recorded traffic" as the reason the
 ...
 * smoothing over.
 *
 * @note This is the one clock here that knows something about market data: that
 *       some venues omit the stamp on some payloads. That is deliberate - the
 *       fact belongs with the clock that has to survive it - but it is why
 *       @c advance_to takes a plain nanosecond count rather than any feed type,
 *       so nothing below the trading engine has to name a feed to keep time.
 *       @see strategy::backtest::session for the caller that supplies it.
 */
class feed_clock {
public:
	/**
	...
	[[nodiscard]] monotonic_time now() const noexcept {
		return monotonic_time{monotonic_clock::duration{
			static_cast<monotonic_clock::rep>(now_ns_)}};
	}
...
public:
	...
	[[nodiscard]] monotonic_time now() const noexcept { return clock_->now(); }
...
static_assert(nanosecond_clock<clock_view>,
