...
public:
     ...
     */
    void place_order(const Order &incoming, std::vector<Trade> &out);

    /// @brief Convenience overload: match @p incoming and return its fills.
    [[nodiscard]] std::vector<Trade> place_order(const Order &incoming);
...
private:
	...
	book_side bid_levels_; ///< descending by price (best = front)
    book_side ask_levels_; ///< ascending by price (best = front)
