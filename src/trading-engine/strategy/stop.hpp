...
public:
	 ...
	 */
	bool arm(const orders::order &o) noexcept {
		if (o.type != orders::order_type::STOP) return false;
	...
	void on_trade(const trade &t, command_writer &out) noexcept {
		for (armed_stop &slot : slots_) {
			...
			orders::order released = slot.resting;
			released.type          = orders::order_type::LIMIT;
	...
	[[nodiscard]] std::optional<orders::order>
	...
	[[nodiscard]] static constexpr bool triggers(const orders::order &o,
