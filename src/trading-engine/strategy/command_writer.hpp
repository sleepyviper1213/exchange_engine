 ...
 * Neither @c Trade nor @c OrderOutcome names a listing — a trade is two order
...
public:
	 ...
	 */
	constexpr command_writer(event::command *first, std::size_t capacity,
	...
	void place(orders::order o) noexcept {
		o.symbol_id = symbol_;
		write(event::command::place(o));
	}

	/// @brief Withdraw the resting order @p id.
	void cancel(order_id_t id) noexcept {
		write(event::command::cancel(symbol_, id));
	}
	...
	void add(side_t side, price_t price, quantity_t volume) noexcept {
		write(event::command::add(symbol_, side, price, volume));
	}
	...
	void reduce(side_t side, price_t price, quantity_t volume) noexcept {
		write(event::command::reduce(symbol_, side, price, volume));
	}

	/// @brief The commands written so far. Valid until @c reset.
	[[nodiscard]] std::span<const event::command> written() const noexcept {
		return {begin_, cur_};
	}
...
private:
	void write(const event::command &cmd) noexcept {
		assert(cur_ != end_ &&
...
public:
	...
	explicit command_batch(symbol_id_t symbol) noexcept
		: writer_(core::util::start_lifetime_as_array<event::command>(
	...
	[[nodiscard]] std::span<const event::command> view() const noexcept {
...
private:
	...
	alignas(event::command)
		std::array<std::byte, Capacity * sizeof(event::command)> storage_;
