...
save_snapshot(const book_manager &books, const std::filesystem::path &path) {
	...
	if (ec)
		return std::unexpected(fmt::format("cannot replace snapshot {}: {}",
										   path.string(),
										   ec.message()));
	...
	books.for_each_listing([&](symbol_id_t symbol, const order_book &book) {
		book.for_each_resting([&](const resting_view &order) {
			staged.push_back(resting_record{.symbol = symbol, .order = order});
			++saved;
	...
	if (!log->sync())
		return std::unexpected(
			fmt::format("cannot sync snapshot {}", path.string()));
...
load_snapshot_reporting(book_manager &books,
	...
	load_report report;
	alignas(resting_record) std::array<std::byte,
									   sizeof(resting_record) * BATCH>
		storage{};
	...
	for (std::uint64_t at = 0; at < total;) {
		...
		for (const resting_record &record : batch) {
			...
			// invented: a snapshot and a partition assignment that disagree is a
			// configuration fault, and handing back a total that does not match
			// the file is how it gets noticed.
			if (book == nullptr) {
				++report.skipped;
				continue;
			}
...
load_snapshot(book_manager &books, const std::filesystem::path &path) {
	return load_snapshot_reporting(books, path).transform(
		[](const load_report &report) { return report.restored; });
}
