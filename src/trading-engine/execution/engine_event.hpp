 ...
 *
 * @par Layout
 * A trivially copyable tagged union, for the same reason @c event::command is:
...
public:
	 ...
	 *
	 * @c event::command deliberately has no default constructor, so that the
	 * active union member always matches the tag. The same reasoning gives this
	 * one *permission* to exist rather than taking it away: a receive buffer is
	 * an array of these, so a default is needed, and zeroing it leaves the tag
	 * and the active member agreeing. Nothing reads a default-constructed
	 * event — the dispatcher only ever looks at the prefix a dequeue filled.
	 */
	...
	bool operator==(const engine_event &) const noexcept;
...
private:
	...
	///        every call site. @see event::command's union for the same
	///        argument.
	union {
