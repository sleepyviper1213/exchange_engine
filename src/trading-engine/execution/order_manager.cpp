...
void order_manager::reject(order_handle handle, reject_reason why) noexcept {
	...
	// every other one. status() reads REJECTED off the bit above rather than out
	// of the quantities, which cannot tell a rejection from a cancel that never
	// filled.
	record->state.cancel();
	retire(handle.slot);
}
...
std::uint32_t order_manager::resolve(order_handle handle) const noexcept {
	...
	// A slot the bump pointer has not reached yet, reachable only by guessing an
	// index. Id 0 is what says nothing lives here.
...
std::uint32_t order_manager::acquire_slot() noexcept {
	// Untouched slots first, so history is only given up once there is genuinely
	// nowhere else to put a new order.
...
void order_manager::retire(std::uint32_t index) noexcept {
	...
	// live + retired never exceeds the slot count, so the ring sized to capacity
	// cannot overrun and the modulo is the only bound it needs.
