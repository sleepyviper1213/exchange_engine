...
// log can tell those two apart, because both look like "no more bytes".
...
/**
 * @brief The engine stopped; no further command in this session will be applied.
 *
 ...
 * must equal @c events_published. A mismatch means the log and the run disagree,
 * which is exactly what a replay is supposed to detect and what a log with no
 * totals in it cannot. @c engine_partition::drain returns the first and
