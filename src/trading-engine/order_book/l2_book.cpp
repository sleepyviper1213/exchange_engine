...
std::vector<l2_book::Level>::iterator
seek(std::vector<l2_book::Level> &levels, Price price, bool descending) {
	if (descending)
		return std::lower_bound(levels.begin(), levels.end(), price,
								[](const l2_book::Level &l, Price p) {
									return l.price > p;
								});
	return std::lower_bound(levels.begin(), levels.end(), price,
							[](const l2_book::Level &l, Price p) {
...
void l2_book::set_level(Side side, Price price, Volume volume) {
	...
	if (volume > 0) levels.insert(at, Level{price, volume});
}
